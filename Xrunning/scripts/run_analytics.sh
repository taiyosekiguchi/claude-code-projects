...
# ログ設定
LOG_FILE="$PROJECT_DIR/logs/analytics_$(date +%Y-%m-%d).log"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "=== Xrunning Analytics 開始: $(date) ==="
...
# 分析パイプライン実行
PYTHONPATH="$PROJECT_DIR" python3 "$PROJECT_DIR/src/analytics_main.py" "$@"

EXIT_CODE=$?
echo "=== Xrunning Analytics 終了: $(date), 終了コード: $EXIT_CODE ==="
exit $EXIT_CODE
